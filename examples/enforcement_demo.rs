...
use elastic_tee_hal::enforcement::policy::{RateLimit, Quota};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let attestation_policy = EntityPolicy::new(
        attestation_service_id.clone(),
        attestation_caps,
    ...
    let crypto_policy = EntityPolicy::new(
        crypto_worker_id.clone(),
        crypto_caps,
    )
    .with_rate_limit("crypto", RateLimit::new(1000)) // 1000 ops/sec
    .with_quota("crypto", Quota {
    ...
    let untrusted_policy = EntityPolicy::new(
        untrusted_id.clone(),
        untrusted_caps,
    ...
    let umbrella_policy = EntityPolicy::new(
        umbrella_id.clone(),
        CapabilitySet::all(),
    ).as_umbrella();
