...
use anyhow::Result;
use elastic_tee_hal::{
    clock::Clock, crypto::CryptoContext, platform::Platform, random::SecureRandom,
...
impl HalHost {
    ...
    pub fn crypto_hash(
        ...
        self.crypto.hash(algo, &data).map_err(|e| e.to_string())
    }
    ...
    pub fn storage_object_read(&mut self, container: u64, key: String) -> Result<Vec<u8>, String> {
        self.storage
    ...
    pub fn clock_system_time(&self) -> Result<SystemTime, String> {
        let time = self.clock.get_system_time().map_err(|e| e.to_string())?;
