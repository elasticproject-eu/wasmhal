...
impl Guest for Component {
    fn add_user(name: String, email: String) -> Result<User, String> {
        ...
        let email_hash = crypto_hal::hash(email.as_bytes(), crypto_hal::HashAlgorithm::Sha256)
            .map_err(|e| format!("Failed to hash email: {}", e))?;

        // 3. Get current timestamp
        let time =
            clock_hal::get_system_time().map_err(|e| format!("Failed to get time: {}", e))?;
    ...
    fn get_user(id: String) -> Result<User, String> {
        ...
        // Deserialize
        let user_record: UserRecord =
            serde_json::from_slice(&user_data).map_err(|e| format!("Data corruption: {}", e))?;
    ...
    fn list_users() -> Result<Vec<String>, String> {
        // Open database
        let container =
            storage_hal::open_container("user-db").map_err(|_| "No users yet".to_string())?;
