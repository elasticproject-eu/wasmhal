...
use super::{AuditEvent, AuditLog, EntityId};
...
impl<T: PlatformInterface> PlatformInterface for AuditedPlatformProvider<T> {
    fn attestation(&self, report_data: &[u8]) -> Result<Vec<u8>, String> {
        ...
        self.audit_log.log(
            AuditEvent::new(self.entity_id.clone(), "platform", "attestation")
    ...
    fn platform_info(&self) -> Result<(String, String, bool), String> {
        ...
        self.audit_log.log(
            AuditEvent::new(self.entity_id.clone(), "platform", "platform_info")
                .with_success(result.is_ok()),
        );
...
impl RateLimiter {
    ...
    fn check_rate_limit(&self) -> bool {
        ...
        *tokens =
            (*tokens + elapsed * self.operations_per_second as f64).min(self.burst_size as f64);
        *last_update = now;
