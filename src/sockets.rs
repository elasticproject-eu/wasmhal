...
impl SocketInterface {
    ...
    pub async fn tcp_accept(&self, listener_handle: SocketHandle) -> HalResult<SocketHandle> {
        ...
        if let Some(SocketWrapper::TcpListener(listener)) = sockets.get_mut(&listener_handle) {
            let (stream, _addr) = listener.accept().await.map_err(|e| {
                HalError::NetworkError(format!("Failed to accept connection: {}", e))
            })?;
    ...
    pub async fn socket_read(
        ...
        match sockets.get_mut(&handle) {
            ...
            Some(SocketWrapper::UdpSocket(socket)) => {
                let (bytes_read, peer_addr) = socket
                    .recv_from(buffer)
                    .await
            ...
            _ => Err(HalError::NotFound(
                "Socket not found or invalid type".to_string(),
            )),
        }
    }
    ...
    pub async fn socket_write(&self, handle: SocketHandle, data: &[u8]) -> HalResult<SocketResult> {
        ...
        match sockets.get_mut(&handle) {
            ...
            _ => Err(HalError::NotFound(
                "Socket not found or invalid type for write".to_string(),
            )),
        }
    }
    ...
    pub async fn udp_send_to(
        ...
        let addr = target_addr
            ...
            .next()
            .ok_or_else(|| {
                HalError::NetworkError("Failed to resolve target address".to_string())
            })?;
    ...
    pub async fn create_tls_client(
        ...
        let client_config = if let Some(config_name) = config_name {
            tls_config
                ...
                .get(config_name)
                .ok_or_else(|| {
                    HalError::NotFound(format!("TLS client config '{}' not found", config_name))
                })?
                .clone()
        } else {
    ...
    pub async fn tls_accept(
        ...
        let tcp_stream = {
            ...
            if let Some(SocketWrapper::TcpStream(stream)) = sockets.remove(&tcp_handle) {
                ...
                return Err(HalError::Internal(
                    "Failed to retrieve TCP stream".to_string(),
                ));
            }
        };
        ...
        let server_config = tls_config
            ...
            .get(config_name)
            .ok_or_else(|| {
                HalError::NotFound(format!("TLS server config '{}' not found", config_name))
            })?
            .clone();
    ...
    pub async fn write_key_context(
        ...
        if let Some(cert) = cert_chain.first() {
            tls_config
                .certificates
                .insert(context_name.to_string(), cert.clone());
        }
        tls_config
            .private_keys
            .insert(context_name.to_string(), private_key.clone_key());
        ...
        if is_server {
            ...
            let mut server_config = ServerConfig::builder()
                ...
                .map_err(|e| {
                    HalError::CryptographicError(format!("Invalid server certificate: {}", e))
                })?;
            ...
            tls_config
                .server_configs
                .insert(context_name.to_string(), Arc::new(server_config));
        } else {
            ...
            let mut client_config = ClientConfig::builder()
                ...
                .map_err(|e| {
                    HalError::CryptographicError(format!("Invalid client certificate: {}", e))
                })?;
            ...
            tls_config
                .client_configs
                .insert(context_name.to_string(), Arc::new(client_config));
        }
        ...
        match sockets.get(&handle) {
            Some(SocketWrapper::TcpListener(listener)) => {
                let local_addr = listener.local_addr().map_err(|e| {
                    HalError::NetworkError(format!("Failed to get local address: {}", e))
                })?;
    ...
    fn parse_pkcs12(
        ...
        pkcs12_data: &[u8],
        _password: &str,
    ) -> HalResult<(Vec<CertificateDer<'static>>, PrivateKeyDer<'static>)> {
...
fn create_placeholder_certificate() -> HalResult<CertificateDer<'static>> {
    ...
    let cert_data = vec![
        ,
...
mod tests {
    ...
    async fn test_tcp_socket_creation() {
        ...
        let handle = socket_interface
            .create_tcp_socket("127.0.0.1:0")
            .await
            .unwrap();
        assert!(handle > 0);
    ...
    async fn test_tcp_connection() {
        ...
        // Create listener
        let listener_handle = socket_interface
            .create_tcp_socket("127.0.0.1:0")
            .await
            .unwrap();
        let listener_info = socket_interface
            .get_socket_info(listener_handle)
            .await
            .unwrap();
        ...
        let write_result = socket_interface_clone
    ...
    async fn test_udp_send_recv() {
        ...
        let socket1 = socket_interface
            .create_udp_socket("127.0.0.1:0")
            .await
            .unwrap();
        let socket2 = socket_interface
            .create_udp_socket("127.0.0.1:0")
            .await
            .unwrap();
        ...
        let send_result = socket_interface
            .udp_send_to(socket1, test_data, &addr2.to_string())
            .await
            .unwrap();
        ...
        let mut buffer = [0u8; 1024];
        let recv_result = socket_interface
            .socket_read(socket2, &mut buffer)
            .await
            .unwrap();
    ...
    async fn test_key_context_operations() {
        ...
        let result = socket_interface
            .write_key_context("test_server", pkcs12_data, "password", true)
            .await;
