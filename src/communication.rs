...
impl CommunicationInterface {
    ...
    pub async fn setup_communication_buffer(
        &self,
        config: BufferConfig,
    ) -> HalResult<CommBufferHandle> {
        ...
        *next_handle += 1;
        ...
        log::info!(
            "Set up communication buffer '{}' with handle {}",
            config.name,
            handle
        );
        Ok(handle)
    ...
    pub async fn push_data_to_buffer(
        ...
        if !buffer
            ...
            && !buffer
                ...
                .contains(&sender.to_string())
        {
            return Err(HalError::PermissionDenied(format!(
                "Sender '{}' does not have write permission",
                sender
            )));
        }
        ...
        let serialized_message = bincode::serialize(&message).map_err(|e| {
            HalError::CommunicationError(format!("Failed to serialize message: {}", e))
        })?;
        ...
        let final_data = if buffer.is_encrypted {
            if let Some(ref key) = buffer.encryption_key {
                ...
                return Err(HalError::CommunicationError(
                    "Buffer is encrypted but no key available".to_string(),
                ));
            }
        } else {
            serialized_message
    ...
    pub async fn read_data_from_buffer(
        ...
        {
            return Err(HalError::PermissionDenied(format!(
                "Reader '{}' does not have read permission",
                reader
            )));
        }
        ...
        // Deserialize message
        let message: CommMessage = bincode::deserialize(&decrypted_data).map_err(|e| {
            HalError::CommunicationError(format!("Failed to deserialize message: {}", e))
        })?;
    ...
    pub async fn clear_buffer(&self, handle: CommBufferHandle, requester: &str) -> HalResult<()> {
        ...
        {
            return Err(HalError::PermissionDenied(format!(
                "Requester '{}' does not have admin permission",
                requester
            )));
        }
    ...
    pub async fn remove_communication_buffer(
        &self,
        handle: CommBufferHandle,
        requester: &str,
    ) -> HalResult<()> {
        ...
        log::info!(
            "Removed communication buffer {} by requester {}",
            handle,
            requester
        );
        Ok(())
    ...
    fn write_bytes_to_buffer(
        &self,
        buffer: &mut CommunicationBuffer,
        data: &[u8],
    ) -> HalResult<()> {
        for &byte in data {
...
mod tests {
    ...
    async fn test_data_push_and_read() {
        ...
        // Read data
        let message = comm_interface
            .read_data_from_buffer(handle, "reader1")
            .await
            .unwrap();
    ...
    async fn test_permission_enforcement() {
        ...
        let result = comm_interface
            .read_data_from_buffer(handle, "unauthorized_reader")
            .await;
        assert!(result.is_err());
    }
    ...
    async fn test_encrypted_buffer() {
        ...
        let message = comm_interface
            .read_data_from_buffer(handle, "reader")
            .await
            .unwrap();
        assert!(message.is_some());
    ...
    async fn test_buffer_listing() {
        ...
        let _handle1 = comm_interface
            .setup_communication_buffer(config1)
            .await
            .unwrap();
        let _handle2 = comm_interface
            .setup_communication_buffer(config2)
            .await
            .unwrap();
    ...
    async fn test_buffer_removal() {
        ...
        // Remove buffer
        comm_interface
            .remove_communication_buffer(handle, "admin")
            .await
            .unwrap();
